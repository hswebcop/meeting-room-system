 ...
 *  會議室管理系統 - app.js（最終版）
 *  首頁：公開月曆（不需登入）
 *  SSO：Intranet 跳轉帶 ?sso_token= 自動登入
 *  點「新增預定」或「登入」後顯示登入視窗
 * ============================================================
 */
...
const SLOT_COUNT  = 22;   // 08:00–19:00
const START_HOUR  = 8;
...
// People Picker 暫存資料
const _peopleData = { attendees:[], observers:[], editAttendees:[], editObservers:[] };
function getPeopleList(key) { return _peopleData[key] || []; }
function setPeopleList(key, arr) { _peopleData[key] = arr; }
...
function apiCall(params) {
  return new Promise((resolve, reject) => {
    ...
    window[cbName] = data => {
      clearTimeout(timeout); delete window[cbName]; script.remove(); resolve(data);
    };
...
function getNowSlot() {
  const n = new Date(); return (n.getHours()-START_HOUR)*2 + (n.getMinutes()>=30?1:0);
}
...
async function trySSOLogin() {
  ...
  try {
    const result = await apiGet({ action: 'ssoLogin', token });
    if (result.success) {
...
function togglePubRoomFilter() {} // 保留，防止舊引用
...
async function loadPublicBookings() {
  ...
  try {
    ...
    pubBookings = Array.isArray(r) ? r.map(b => ({
      ...
      attendees: (!b.attendees || b.attendees === 'undefined') ? '[]' : b.attendees,
      observers: (!b.observers || b.observers === 'undefined') ? '[]' : b.observers,
    })) : [];
    try {
  ...
  finally { document.getElementById('pubLoadingBar').classList.add('hidden'); }
}
...
function goThisMonth() { currentMonth = new Date(); currentMonth.setDate(1); renderMonthCal(); }
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    if (isValid) {
      ...
      dayEvts.slice(0, MAX_SHOW).forEach(b => {
        html += `<div class="mcal-event ${ROOM_COLORS[b.roomIdx]}" onclick="showPubEventDetail(${b.id})" title="${b.subject}">
...
function showPubEventDetail(bookingId) {
  const b = pubBookings.find(x => x.id===bookingId); if (!b) return;
  const user = allUsers.find(u => u.id===b.userId);
  let attendees=[], observers=[];
  try { attendees = JSON.parse(b.attendees||'[]'); } catch(e) {}
  try { observers = JSON.parse(b.observers||'[]'); } catch(e) {}
  const fmtPeople = arr => arr.length ? arr.map(p=>p.name||p.email).join('、') : '（無）';
  document.getElementById('pubEventModalBody').innerHTML = `
    ...
    <div class="event-detail-row"><span class="event-detail-label">預定人</span>${user?.cname||user?.name||b.userId}</div>
    ${b.desc?`<div class="event-detail-row"><span class="event-detail-label">說明</span><span style="color:var(--text2)">${b.desc}</span></div>`:''}
    <div class="event-detail-row"><span class="event-detail-label">與會者</span><span style="color:var(--text2)">${fmtPeople(attendees)}</span></div>
    <div class="event-detail-row"><span class="event-detail-label">列席者</span><span style="color:var(--text2)">${fmtPeople(observers)}</span></div>
  `;
  document.getElementById('pubEventModal').classList.remove('hidden');
}

function showDayDetail(ds) {
  const dayEvts = pubBookings.filter(b => b.date===ds && pubRoomFilters.includes(b.roomIdx)).sort((a,b)=>a.startSlot-b.startSlot);
    ...
    dayEvts.map(b => {
      ...
      return `<div class="day-detail-item ${ROOM_COLORS[b.roomIdx]}" onclick="showPubEventDetail(${b.id})">
        ...
        <div class="day-detail-info">
          ...
          <div class="day-detail-meta">${ROOMS[b.roomIdx]} · ${u?.cname||u?.name||b.userId}</div>
        </div>
      </div>`;
    }).join('');
  document.getElementById('pubEventModal').classList.remove('hidden');
...
function requireLogin(action, extraArg) {
  if (currentUser) { afterLoginAction(action, extraArg); return; }
...
function afterLoginAction(action, extraArg) {
  if (action === 'book') {
    showPage('book');
    if (extraArg) { document.getElementById('bookDate').value = extraArg; updateAvailableSlots(); }
...
async function doLogin() {
  ...
  try {
    ...
    if (!result.success) { errEl.textContent=result.message||'登入失敗'; errEl.classList.remove('hidden'); return; }
    if (authMode==='admin'&&result.user.role!=='admin') {
      errEl.textContent='此帳號沒有管理員權限。'; errEl.classList.remove('hidden'); return;
    }
    currentUser = result.user;
    ...
    await initApp();
    if (pendingAction) { afterLoginAction(pendingAction.action, pendingAction.extraArg); pendingAction=null; }
  } catch(e) { errEl.textContent='連線失敗：'+e.message; errEl.classList.remove('hidden'); }
  finally { btn.innerHTML='<span id="loginBtnText">登入</span>'; btn.disabled=false; }
}
...
function confirmLogout() {
  ...
  document.getElementById('appScreen').classList.add('hidden');
  document.getElementById('publicScreen').classList.remove('hidden');
  updatePubLoginState(); loadPublicBookings();
}
/* 從首頁進入系統（已登入狀態） */
function enterApp() {
...
function populateBookingUserSelect() {
  ...
  else { sel.disabled=false; sel.value=val||currentUser.id; }
}
...
function renderCalendar() {
  ...
  const today=formatDate(new Date()), nowSlot=getNowSlot();
  ...
  for (let s=0;s<SLOT_COUNT;s++) html+=`<div class="cal-time-slot">${s%2===0?slotToTime(s):''}</div>`;
  ...
  for (let i=0;i<7;i++) {
    ...
    dayEvts.forEach(b => {
      const u=getUser(b.userId), col=columns[b.id], cols=col.total;
      const gap=1, pct=(100-gap*(cols-1))/cols;
      const left=col.index*(pct+gap), width=pct;
      html+=`<div class="cal-event ${ROOM_COLORS[b.roomIdx]}"
        style="top:${b.startSlot*40}px;height:${(b.endSlot-b.startSlot)*40-3}px;left:${left.toFixed(1)}%;width:${width.toFixed(1)}%;"
...
function assignColumns(events) {
  ...
  events.forEach(ev => {
    ...
    for (let c=0;c<cols.length;c++) {
      if (cols[c]<=ev.startSlot) { cols[c]=ev.endSlot; result[ev.id]={index:c,total:0}; placed=true; break; }
    }
    if (!placed) { cols.push(ev.endSlot); result[ev.id]={index:cols.length-1,total:0}; }
  });
  events.forEach(ev => {
...
function calSlotClick(date, slot, isPast) {
  ...
  setTimeout(() => { const s=document.getElementById('bookStartTime'); if(s){s.value=slot;onStartTimeChange();} }, 80);
}
...
function isSlotTaken(slot, ranges) { return ranges.some(b=>slot>=b.startSlot&&slot<b.endSlot); }

function buildStartOptions(roomIdx, date) {
  const today=formatDate(new Date()), nowSlot=today===date?getNowSlot():-1;
...
function onStartTimeChange() {
  ...
  if (!room||!date||startVal==='') { document.getElementById('bookEndTime').innerHTML='<option value="">-- 先選開始時間 --</option>'; return; }
  buildEndOptions(parseInt(room), date, parseInt(startVal));
  document.getElementById('bookEndTime').value='';
  document.getElementById('timeConflictHint').classList.add('hidden');
}
function onEndTimeChange() {
  const room=document.getElementById('bookRoom').value, date=document.getElementById('bookDate').value;
  const startVal=document.getElementById('bookStartTime').value, endVal=document.getElementById('bookEndTime').value;
  const hintEl=document.getElementById('timeConflictHint');
  if (!room||!date||startVal===''||endVal==='') { hintEl.classList.add('hidden'); return; }
  const conflicts=getConflicts(parseInt(room), date, parseInt(startVal), parseInt(endVal));
  if (conflicts.length>0) {
    hintEl.innerHTML=conflicts.map(c => {
      const u=getUser(c.userId);
      return `⚠️ <strong>${ROOMS[parseInt(room)]}</strong> 在 <strong>${slotToTime(c.startSlot)}–${slotToTime(c.endSlot)}</strong> 已由 <strong>${u?.cname||u?.name||c.userId}</strong> 預定「${c.subject}」`;
    }).join('<br>');
    hintEl.classList.remove('hidden');
  } else { hintEl.classList.add('hidden'); }
}
function getConflicts(roomIdx, date, startSlot, endSlot) {
  return allBookings.filter(b=>b.roomIdx===roomIdx&&b.date===date&&!(endSlot<=b.startSlot||startSlot>=b.endSlot));
}
function renderBookedList(roomIdx, date) {
  const el=document.getElementById('bookedList');
  const list=allBookings.filter(b=>b.roomIdx===roomIdx&&b.date===date).sort((a,b)=>a.startSlot-b.startSlot);
  if (!list.length) { el.classList.add('hidden'); return; }
  el.classList.remove('hidden');
  el.innerHTML=`<div class="booked-list-title">当日已預定時段</div>`+
    list.map(b => {
      ...
      return `<div class="booked-item">
        ...
        <span class="booked-user">${u?.cname||u?.name||b.userId}</span>
      </div>`;
    }).join('');
}
...
function resetBookForm() {
  ...
  setPeopleList('attendees',[]); renderPeopleTags('bookAttendeeTags','attendees');
  setPeopleList('observers',[]); renderPeopleTags('bookObserverTags','observers');
}

async function submitBooking() {
  const errEl=document.getElementById('bookError'), succEl=document.getElementById('bookSuccess');
  ...
  const attendees=getPeopleList('attendees');
  const observers=getPeopleList('observers');
  ...
  const roomIdx=parseInt(room), startSlot=parseInt(startVal), endSlot=parseInt(endVal);
  const conflicts=getConflicts(roomIdx, date, startSlot, endSlot);
  if (conflicts.length>0) {
    const lines=conflicts.map(c=>{const cu=getUser(c.userId);return `• ${ROOMS[roomIdx]} ${slotToTime(c.startSlot)}–${slotToTime(c.endSlot)} 已由「${cu?.cname||cu?.name||c.userId}」預定：${c.subject}`;}).join('<br>');
    showAlert(errEl,`⚠️ 時段衝突，無法預定！<br><br>${lines}<br><br>請重新選擇其他時段或會議室。`); return;
  }
  const btn=document.getElementById('bookSubmitBtn');
  btn.innerHTML='<span class="spinner"></span> 預定中...'; btn.disabled=true;
  try {
    const res=await apiPost({action:'addBooking',userId,roomIdx,date,startSlot,endSlot,subject,desc,attendees:JSON.stringify(attendees),observers:JSON.stringify(observers)});
    if (!res.success) {
      if (res.conflict) { showAlert(errEl,`⚠️ 預定失敗！此時段剛被他人搶先預定。（${res.message}）`); await refreshData(); }
      else showAlert(errEl,res.message||'預定失敗，請稍後再試。');
      return;
    }
    const user=getUser(userId);
    try {
      await apiPost({action:'sendEmail',email:user?.email||'',userName:user?.cname||user?.name||userId,
        subject,desc,roomIdx,date,startTime:slotToTime(startSlot),endTime:slotToTime(endSlot),
        attendees:JSON.stringify(attendees),observers:JSON.stringify(observers)});
      const total=1+attendees.length+observers.length;
      succEl.innerHTML=`✅ 預定成功！已發送行事曆邀請給 ${total} 位成員。`;
...
function showEventDetail(bookingId) {
  ...
  const user=getUser(b.userId), canEdit=currentUser.role==='admin'||currentUser.id===b.userId;
  let attendees=[], observers=[];
  try { attendees=JSON.parse(b.attendees||'[]'); } catch(e) {}
  try { observers=JSON.parse(b.observers||'[]'); } catch(e) {}
  const fmtPeople=arr=>arr.length?arr.map(p=>p.name||p.email).join('、'):'（無）';
...
function openEditModal(bookingId) {
  ...
  try { attendees=JSON.parse(b.attendees||'[]'); } catch(e) {}
  try { observers=JSON.parse(b.observers||'[]'); } catch(e) {}
  setPeopleList('editAttendees', attendees); renderPeopleTags('editAttendeeTags','editAttendees');
  setPeopleList('editObservers', observers); renderPeopleTags('editObserverTags','editObservers');
  ...
  setTimeout(() => {
    ...
    setTimeout(()=>{ document.getElementById('editEndTime').value=b.endSlot; }, 50);
  }, 50);
  document.getElementById('editModal').classList.remove('hidden');
}

function buildEditStartOptions(roomIdx, date, excludeId) {
  const today=formatDate(new Date()), ranges=allBookings.filter(b=>b.roomIdx===roomIdx&&b.date===date&&b.id!==excludeId);
...
function onEditEndTimeChange() {
  ...
  const conflicts=allBookings.filter(b=>b.roomIdx===roomIdx&&b.date===date&&b.id!==id&&!(parseInt(endVal)<=b.startSlot||parseInt(startVal)>=b.endSlot));
  if (conflicts.length>0) {
    hintEl.innerHTML=conflicts.map(c=>{const u=getUser(c.userId);return `⚠️ <strong>${slotToTime(c.startSlot)}–${slotToTime(c.endSlot)}</strong> 已由 <strong>${u?.cname||u?.name||c.userId}</strong> 預定「${c.subject}」`;}).join('<br>');
    hintEl.classList.remove('hidden');
  } else { hintEl.classList.add('hidden'); }
}
...
async function saveEditBooking() {
  ...
  const attendees=getPeopleList('editAttendees');
  const observers=getPeopleList('editObservers');
  ...
  if (endVal==='')   { showAlert(errEl,'請選擇結束時間。'); return; }
  if (!subject)    { showAlert(errEl,'請輸入會議主旨。');   return; }
  const startSlot=parseInt(startVal), endSlot=parseInt(endVal);
  const conflicts=allBookings.filter(b=>b.roomIdx===roomIdx&&b.date===date&&b.id!==id&&!(endSlot<=b.startSlot||startSlot>=b.endSlot));
  if (conflicts.length>0) {
    const lines=conflicts.map(c=>{const cu=getUser(c.userId);return `• ${slotToTime(c.startSlot)}–${slotToTime(c.endSlot)} ${cu?.cname||cu?.name||c.userId}：${c.subject}`;}).join('<br>');
    showAlert(errEl,`⚠️ 時段衝突，無法儲存！<br><br>${lines}<br><br>請重新選擇時段。`); return;
  }
  const btn=document.getElementById('editSaveBtn');
  btn.innerHTML='<span class="spinner"></span>'; btn.disabled=true;
  try {
    const res=await apiPost({action:'updateBooking',id,roomIdx,date,startSlot,endSlot,subject,desc,attendees:JSON.stringify(attendees),observers:JSON.stringify(observers)});
    if (!res.success) {
      if (res.conflict) { showAlert(errEl,'⚠️ 時段剛被他人搶先預定，請重新選擇。'); await refreshData(); }
      else showAlert(errEl,res.message);
      return;
    }
    ...
    if (user?.email) {
      try {
        await apiPost({action:'sendEmail',email:user.email,userName:user.cname||user.name||user.id,
          subject,desc,roomIdx,date,startTime:slotToTime(startSlot),endTime:slotToTime(endSlot),
          isUpdate:'true',attendees:JSON.stringify(attendees),observers:JSON.stringify(observers)});
      } catch {}
    }
    closeModal('editModal'); await refreshData(); showToast('會議已更新，通知已發送。');
...
function renderMyBookings() {
  ...
  const today=formatDate(new Date());
  const myB=allBookings.filter(b=>b.userId===currentUser.id).sort((a,b)=>a.date.localeCompare(b.date)||a.startSlot-b.startSlot);
  if (!myB.length) {
    container.innerHTML=`<div class="alert alert-info">您目前沒有預定任何會議。<br><button class="btn btn-primary" style="margin-top:0.8rem;max-width:200px;" onclick="showPage('book')">立即預定會議室</button></div>`;
    return;
  }
  container.innerHTML=myB.map(b => {
    const isPast=b.date<today;
    let attendees=[]; try{attendees=JSON.parse(b.attendees||'[]');}catch(e){}
    return `<div class="booking-card">
      ...
      <div class="booking-info">
        ...
        <div class="booking-meta">
          ...
          ${attendees.length?`<span>與會者：${attendees.map(p=>p.name||p.email).join('、')}</span>`:''}
        </div>
      </div>
      <div class="booking-actions">
        ${!isPast?`<button class="btn btn-secondary btn-sm" onclick="openEditModal(${b.id})">編輯</button>
...
function renderAdminBookings() {
  ...
  tbody.innerHTML=sorted.map(b => {
    ...
    return `<tr>
      ...
      <td>${b.subject}</td><td>${user?.cname||user?.name||b.userId}</td>
...
function renderAdminUsers() {
  ...
  tbody.innerHTML=allUsers.map(u=>`<tr>
    <td class="mono">${u.id}</td><td>${u.cname||''}</td><td>${u.name}</td>
...
async function saveUser() {
  ...
  try {
    ...
    const payload=mode==='add'?{action,id,cname,name,email,password:pwd,role}:{action,id,cname,name,email,role,...(pwd?{password:pwd}:{})};
...
function loadProfile() {
  ...
  if (fresh) { currentUser.cname=fresh.cname||''; currentUser.name=fresh.name; currentUser.email=fresh.email; }
  const u=currentUser;
...
/* ──────────────────────────────────────────
   PEOPLE PICKER — 與會者 / 列席者
────────────────────────────────────────── */
function renderPeopleTags(tagsElId, key) {
  const el=document.getElementById(tagsElId);
  if (!el) return;
  const list=getPeopleList(key);
  el.innerHTML=list.map((p,i)=>
    `<span class="people-tag">${p.name||p.email}<button onclick="removePerson('${key}','${tagsElId}',${i})" title="移除">×</button></span>`
  ).join('');
}
function removePerson(key, tagsElId, idx) {
  const list=getPeopleList(key); list.splice(idx,1); renderPeopleTags(tagsElId,key);
}
function onPeopleInput(inputId, suggestId) {
  const val=document.getElementById(inputId).value.trim().toLowerCase();
  const el=document.getElementById(suggestId);
  if (!val||val.length<1) { el.classList.add('hidden'); return; }
  const matches=allUsers.filter(u=>{
    const cn=(u.cname||'').toLowerCase(), nm=(u.name||'').toLowerCase();
    const em=(u.email||'').toLowerCase(), id=(u.id||'').toLowerCase();
    return cn.includes(val)||nm.includes(val)||em.includes(val)||id.includes(val);
  }).slice(0,8);
  if (!matches.length) { el.classList.add('hidden'); return; }
  el.innerHTML=matches.map(u=>
    `<div class="people-suggest-item" onclick='selectPerson("${inputId}","${suggestId}",${JSON.stringify({id:u.id,name:u.cname||u.name,email:u.email}).replace(/'/g,"&#39;")})'>
      <span class="suggest-name">${u.cname||u.name}</span><span class="suggest-email">${u.email}</span>
    </div>`
  ).join('');
  el.classList.remove('hidden');
}
function selectPerson(inputId, suggestId, user) {
  const key=pickKeyFromInputId(inputId), tagsId=inputId.replace('Input','Tags');
  const list=getPeopleList(key);
  if (!list.find(p=>p.email===user.email)) { list.push({name:user.name,email:user.email}); renderPeopleTags(tagsId,key); }
  document.getElementById(inputId).value='';
  document.getElementById(suggestId).classList.add('hidden');
}
function addPeopleByInput(inputId, tagsId, key) {
  const val=document.getElementById(inputId).value.trim();
  if (!val) return;
  const emailReg=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (emailReg.test(val)) {
    const list=getPeopleList(key);
    if (!list.find(p=>p.email===val)) { list.push({name:val.split('@')[0],email:val}); renderPeopleTags(tagsId,key); }
  } else {
    const match=allUsers.find(u=>(u.cname||'').includes(val)||(u.name||'').includes(val)||u.id===val);
    if (match) {
      const list=getPeopleList(key);
      if (!list.find(p=>p.email===match.email)) { list.push({name:match.cname||match.name,email:match.email}); renderPeopleTags(tagsId,key); }
    } else { showToast('請輸入有效的 email 或成員名稱','error'); return; }
  }
  document.getElementById(inputId).value='';
  document.getElementById(inputId.replace('Input','Suggest'))?.classList.add('hidden');
}
function onPeopleKeydown(event, inputId, tagsId, key) {
  if (event.key==='Enter') { event.preventDefault(); addPeopleByInput(inputId,tagsId,key); }
}
function pickKeyFromInputId(inputId) {
  const map={'bookAttendeeInput':'attendees','bookObserverInput':'observers','editAttendeeInput':'editAttendees','editObserverInput':'editObservers'};
  return map[inputId]||'';
}
document.addEventListener('click', e => {
  document.querySelectorAll('.people-suggest').forEach(el => { if (!el.contains(e.target)) el.classList.add('hidden'); });
});
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (notConfigured) { document.getElementById('pubSetupNotice').classList.remove('hidden'); renderMonthCal(); return; }
  document.getElementById('pubSetupNotice').classList.add('hidden');

  // 嘗試 SSO 自動登入（Intranet 跳轉）
